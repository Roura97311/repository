...
AddonWorker = collections.namedtuple(
...
# ============================================================
# ADDONS UTILISES PAR LE REPOSITORY AEON MIRO NOX
# ============================================================
...
def get_metadata_basenames(addon_metadata):
    return (
        [(basename, basename) for basename in METADATA_BASENAMES] +
        [(
            'changelog.txt',
...
def is_url(addon_location):
    return bool(
        re.match(r'[A-Za-z0-9+.-]+://.', addon_location)
    )


def parse_metadata(metadata_file):
    """
    Parse addon.xml metadata.
    """
    ...
    # Validate add-on ID.
    if (
        addon_metadata.id is None or
        re.search('[^a-z0-9._-]', addon_metadata.id)
    ):
        raise RuntimeError(
            'Invalid addon ID: ' + str(addon_metadata.id)
        )

    # Validate version.
    if (
        addon_metadata.version is None or
        not re.match(
            r'\d+\.\d+\.\d+$',
            addon_metadata.version
        )
...
def copy_metadata_files(
    ...
    for (
        source_basename,
        target_basename
    ) in get_metadata_basenames(addon_metadata):
...
def fetch_addon_from_git(
    ...
    # Optional alternate addon ID/name.
    ...
    return fetch_addon_from_folder(
        addon_temp,
        target_folder
    )


def fetch_addon_from_folder(
        raw_addon_location,
        target_folder):

    try:

        addon_location = os.path.abspath(
            raw_addon_location
        )
        ...
        addon_metadata = parse_metadata(
            metadata_path
        )

        # ----------------------------------------------------
        # Ignore addons that do not belong to Aeon Miro Nox.
        # ----------------------------------------------------
        ...
        cur_metadata_path = os.path.join(
            addon_target_folder,
            INFO_BASENAME
        )
        ...
        if os.path.exists(cur_metadata_path):
            ...
            cur_metadata = parse_metadata(
                cur_metadata_path
            )

            if (
                cur_metadata.version ==
                addon_metadata.version
            ):

                print(
                    "Addon %s already has version %s "
                    "on the repo, skipping..."
                    % (
                        addon_metadata.id,
                        addon_metadata.version
                    )
...
def buildskintextures(addon_folder):
    """
    Optional skin texture builder.
    Kept from the original 1.2.2.
    """
...
def fetch_addon_from_zip(
    ...
    with zipfile.ZipFile(
        ...
        if len(roots) != 1:
            raise RuntimeError(
                'Archive should contain one directory'
            )
        ...
        if not root:
            raise RuntimeError(
                'Archive should contain a directory'
            )
    ...
    if (
        not samefile(
            os.path.dirname(addon_location),
            addon_target_folder
        )
        or
        os.path.basename(addon_location)
        != archive_basename
    ):
...
def create_repository(
    ...
    for worker in workers:
        ...
        try:
            ...
            result = next(
                iter(worker.result_slot)
            )

        except StopIteration:

            raise RuntimeError(
                'Addon worker did not report result'
            )
        ...
        # Ignore addons outside our repository.
        if (
            result.addon_metadata.id
            in ALLOWED_ADDONS
        ):

            metadata.append(
                result.addon_metadata
            )

    # --------------------------------------------------------
    # Remove duplicate addon IDs.
    # --------------------------------------------------------
    ...
    metadata = list(
        unique_metadata.values()
    )

    # Sort catalog for predictable output.
    metadata.sort(
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description=(
            'Create a Kodi add-on repository '
            'from add-on sources'
        )
    )
    ...
    parser.add_argument(
        ...
        '-i',
        help=(
            'Path for the addons.xml file '
            '[DATADIR/addons.xml or '
    ...
    parser.add_argument(
        ...
        '-c',
        help=(
            'Path for the addons.xml.md5 file '
    ...
    parser.add_argument(
        ...
        metavar='ADDON',
        help=(
            'Location of the add-on: either a path '
            'to a local folder or to a zip archive '
            'or a URL for a Git repository'
        )
    )
